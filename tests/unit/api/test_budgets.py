...
class TestCurrentSpend:
    def test_returns_four_tiers(self):
        resp = client.get(f"{PREFIX}/current-spend")
...
class TestBreakerEvents:
    ...
    def test_filter_by_tier(self):
        resp = client.get(f"{PREFIX}/events", params={"tier": "llm_inference"})
        assert resp.status_code == 200
        for event in resp.json()["events"]:
            assert event["tier"] == "llm_inference"

    def test_filter_unknown_tier_returns_empty(self):
        resp = client.get(f"{PREFIX}/events", params={"tier": "nonexistent"})
        assert resp.status_code == 200
        assert resp.json()["events"] == []
        assert resp.json()["total"] == 0

    def test_pagination_limit(self):
        resp = client.get(f"{PREFIX}/events", params={"limit": 2})
        assert resp.status_code == 200
        assert len(resp.json()["events"]) <= 2

    def test_pagination_offset(self):
        all_resp = client.get(f"{PREFIX}/events")
        offset_resp = client.get(f"{PREFIX}/events", params={"offset": 2})
