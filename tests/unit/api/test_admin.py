...
class TestListUsers:
    ...
    def test_filter_by_role(self, client):
        resp = client.get("/api/v1/admin/users?role=ADMIN")
        assert resp.status_code == 200
        data = resp.json()
        assert all(u["role"] == "ADMIN" for u in data["users"])

    def test_filter_by_status(self, client):
        resp = client.get("/api/v1/admin/users?status=active")
        assert resp.status_code == 200
        data = resp.json()
...
class TestInviteUser:
    def test_invite_new_user(self, client):
    ...
    def test_invited_user_appears_in_list(self, client):
...
class TestUpdateRole:
    def test_update_role(self, client):
        resp = client.patch("/api/v1/admin/users/usr-ops-001/role", json={"role": "COMPLIANCE"})
        assert resp.status_code == 200
        assert resp.json()["role"] == "COMPLIANCE"

    def test_update_role_invalid(self, client):
        resp = client.patch("/api/v1/admin/users/usr-ops-001/role", json={"role": "FAKE"})
        assert resp.status_code == 400

    def test_update_role_user_not_found(self, client):
        resp = client.patch("/api/v1/admin/users/usr-nonexist/role", json={"role": "ADMIN"})
...
class TestDeactivateActivate:
    def test_deactivate_user(self, client):
        resp = client.post("/api/v1/admin/users/usr-ops-001/deactivate")
        assert resp.status_code == 200
        assert resp.json()["status"] == "deactivated"

    def test_deactivate_not_found(self, client):
        resp = client.post("/api/v1/admin/users/usr-nonexist/deactivate")
        assert resp.status_code == 404

    def test_activate_user(self, client):
        client.post("/api/v1/admin/users/usr-ops-001/deactivate")
        resp = client.post("/api/v1/admin/users/usr-ops-001/activate")
        assert resp.status_code == 200
        assert resp.json()["status"] == "active"

    def test_activate_not_found(self, client):
        resp = client.post("/api/v1/admin/users/usr-nonexist/activate")
        assert resp.status_code == 404
...
class TestSSOConfig:
    def test_get_sso_config(self, client):
        resp = client.get("/api/v1/admin/sso")
        assert resp.status_code == 200
        data = resp.json()
        assert "oidc_google_enabled" in data
        assert "saml_microsoft_enabled" in data

    def test_update_sso_config(self, client):
    ...
    def test_update_sso_partial(self, client):
        resp = client.put("/api/v1/admin/sso", json={
