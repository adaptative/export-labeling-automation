...
class TestListAuditEntries:
    ...
    def test_pagination_limit(self):
        resp = client.get(PREFIX, params={"limit": 5})
        assert resp.status_code == 200
        assert len(resp.json()["entries"]) == 5

    def test_pagination_offset(self):
        all_resp = client.get(PREFIX, params={"limit": 100})
        offset_resp = client.get(PREFIX, params={"offset": 3, "limit": 100})
    ...
    def test_search_by_actor(self):
        resp = client.get(PREFIX, params={"search": "sarah"})
        assert resp.status_code == 200
        for entry in resp.json()["entries"]:
            assert "sarah" in entry["actor"].lower() or "sarah" in entry["detail"].lower()

    def test_search_by_resource_id(self):
        resp = client.get(PREFIX, params={"search": "PO-2065"})
        assert resp.status_code == 200
        assert resp.json()["total"] > 0

    def test_filter_actor_type(self):
        resp = client.get(PREFIX, params={"actor_type": "agent"})
        assert resp.status_code == 200
        for entry in resp.json()["entries"]:
            assert entry["actor_type"] == "agent"

    def test_filter_action(self):
        resp = client.get(PREFIX, params={"action": "CREATE"})
        assert resp.status_code == 200
        for entry in resp.json()["entries"]:
            assert entry["action"] == "CREATE"

    def test_sort_desc_default(self):
        resp = client.get(PREFIX, params={"limit": 100})
        entries = resp.json()["entries"]
        timestamps = [e["timestamp"] for e in entries]
        assert timestamps == sorted(timestamps, reverse=True)

    def test_sort_asc(self):
        resp = client.get(PREFIX, params={"sort_order": "asc", "limit": 100})
        entries = resp.json()["entries"]
        timestamps = [e["timestamp"] for e in entries]
        assert timestamps == sorted(timestamps)

    def test_combined_filters(self):
        resp = client.get(PREFIX, params={"actor_type": "user", "action": "APPROVE"})
        assert resp.status_code == 200
        for entry in resp.json()["entries"]:
...
class TestGetAuditEntry:
    def test_get_existing(self):
        resp = client.get(f"{PREFIX}/aud-001")
        assert resp.status_code == 200
        assert resp.json()["id"] == "aud-001"

    def test_get_not_found(self):
        resp = client.get(f"{PREFIX}/aud-999")
