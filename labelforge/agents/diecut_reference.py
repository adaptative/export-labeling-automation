...
# ── Pipeline adapter ───────────────────────────────────────────────────────
#
# The Composer activity calls these helpers to turn a fused-item dict
# (shape produced by fuse_data_activity) into the ``item`` dict shape
# the reference ``generate_diecut()`` expects. Kept below the original
# script so the script still runs standalone.
...
_DRAWING_KEYWORDS = (
    ...
    ("riser", "wood_riser"),
    ("jug", "jug_handles"),
    ("tall", "tall_handle_vase"),
    ("vase", "vase_handles"),
)


def infer_drawing_key(description: str) -> str:
    """Map a free-text product description to a ``DRAWINGS`` key.

    Falls back to ``vase_handles`` when no keyword matches — it's the
    most generic outline in the set so a mismatch still renders
    something reasonable rather than an empty drawing frame.
    """
    desc = (description or "").lower()
    for kw, key in _DRAWING_KEYWORDS:
        if kw in desc:
            return key
    return "vase_handles"


def generate_diecut_for_payload(fused_item: dict, order_payload: dict | None = None) -> str:
    """Build the reference ``item`` dict from a fused_item + order payload
    and return the SVG produced by :func:`generate_diecut`.

    Only a subset of keys are required — missing ones fall back to
    sensible defaults so legacy items without full metadata still
    render a valid label.
    """
    order_payload = order_payload or {}
    dims = fused_item.get("product_dims") or {}
    # Prefer explicit box dims; fall back to product_dims → 10" cube.
    box_L = float(fused_item.get("box_L") or dims.get("length") or 10.0)
    box_W = float(fused_item.get("box_W") or dims.get("width") or 10.0)
    box_H = float(fused_item.get("box_H") or dims.get("height") or 10.0)
    case_qty_raw = fused_item.get("case_qty")
    case_qty = (
        f"{int(case_qty_raw)} PCS" if isinstance(case_qty_raw, (int, float))
        else str(case_qty_raw or "")
    )
    # PO number: strip common prefixes so the label shows the bare PO.
    po_raw = str(
        fused_item.get("po_number")
        or order_payload.get("po_number")
        or fused_item.get("po_no")
        or ""
    )
    po_no = po_raw.split("-")[-1] if "-" in po_raw else po_raw

    adapted = {
        "item_no": str(fused_item.get("item_no") or ""),
        "description": str(fused_item.get("description") or ""),
        "case_qty": case_qty,
        "total_qty": int(fused_item.get("total_qty") or 0),
        "total_cartons": int(fused_item.get("total_cartons") or 0),
        "box_L": box_L,
        "box_W": box_W,
        "box_H": box_H,
        "cbm": float(fused_item.get("cbm") or 0.0),
        "upc": str(fused_item.get("upc") or "000000000000"),
        "gtin": str(fused_item.get("gtin") or ""),
        "po_no": po_no or "N/A",
        "drawing": fused_item.get("drawing") or infer_drawing_key(
            fused_item.get("description") or ""
        ),
        "material": str(fused_item.get("material") or "").title() or "—",
        "finish": str(fused_item.get("finish") or "").title() or "—",
        "warnings": list(fused_item.get("applicable_warning_labels") or []),
    }
    return generate_diecut(adapted)
